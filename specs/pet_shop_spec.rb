...
class TestPetShop < Minitest::Test
  ...
  def test_pet_shop_name_not_hardcoded
   assert_equal("Petshop Galaxy", pet_shop_name({name: "Petshop Galaxy"}))
  end
  ...
  def test_pets_sold_not_hardcoded
    sold = pets_sold({admin: {pets_sold: 3
      }})
    assert_equal(3, sold)
  end
  ...
  def test_increase_pets_sold_3
    increase_pets_sold(@pet_shop,3)
    sold = pets_sold(@pet_shop)
    assert_equal(3, sold)
  end
  ...
  def test_stock_count
    count = stock_count({pets: ["pet 1","pet 2" ,3,4]})
  ...
  def test_all_pets_by_breed__found_not_hardcoded_array
    pets = pets_by_breed(@pet_shop, "British Shorthair")
    assert_equal(2, pets.count)
  end
